...
async function loadStats(){
  // Minimal: call a protected endpoint if available — otherwise show local demo stats
  const token = localStorage.getItem('authToken');
  if(!token){ statsEl.textContent='No stats (not authenticated)'; return; }
  try{
    // if backend had /me or /stats we could call it. For now show demo stats.
    statsEl.textContent = 'Total donations: 24 | Food saved: 156.5 kg | Points: 450';
  }catch(e){ statsEl.textContent='Error loading stats'; }
}
...
async function handlePostDonation(e){
  ...
  // send to backend if donation API exists
  try{
